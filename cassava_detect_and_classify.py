...
Combines:
1. Leaf detection using YOLOv8
2. Disease classification on detected leaves
...
class CassavaDiseasePipeline:
    ...
    def process_image(self, image_path, conf_threshold=0.4):
        ...
        # Extract each detected leaf and classify it
        leaf_results = []
        ...
        for i, box in enumerate(detections.boxes.xyxy):
            ...
            leaf_img = img.crop((x1, y1, x2, y2))
            
            # Classify the leaf
            disease, disease_conf = self._classify_leaf(leaf_img)
        ...
        return img, leaf_results
    
    def visualize_results(self, img, leaf_results, save_path=None):
        ...
        # Plot each leaf with its classification
        for result in leaf_results:
            x1, y1, x2, y2 = result['bbox']
            disease = result['disease']
            det_conf = result['detection_conf']
            dis_conf = result['disease_conf']
        ...
        # Set title
        filename = os.path.basename(save_path) if save_path else "Cassava Analysis"
        ax.set_title(f"Detected {len(leaf_results)} leaves with disease classification")
        ax.axis('off')
        plt.tight_layout()
    ...
    def process_folder(self, image_folder, output_folder="disease_analysis", num_images=None):
        ...
        for i, img_path in enumerate(image_paths):
            ...
            img, leaf_results = self.process_image(img_path)
            ...
            self.visualize_results(img, leaf_results, output_path)
        
        print(f"All images processed. Results saved to {output_folder}/")

def main():
    # Parse command-line arguments
    parser = argparse.ArgumentParser(description='Cassava Leaf Disease Classification Pipeline')
    parser.add_argument('--detection_model', 
                        type=str, 
                        default='cassava_detection_new/yolo_yolov8n_optimized/weights/best.pt',
                        help='Path to YOLO detection model')
    parser.add_argument('--classification-model', type=str, 
                        default='models/resnet50_cassava.pth',
                        help='Path to classification model')
    parser.add_argument('--test-folder', type=str, 
                        default='data/Cassava_Leaf_Detector_v1i_yolov8/test/images',
                        help='Folder containing test images')
    parser.add_argument('--output-folder', type=str, 
                        default='disease_analysis',
    ...
    # Create the pipeline
    pipeline = CassavaDiseasePipeline(detection_model_path, classification_model_path)
    ...
    pipeline.process_folder(test_folder, output_folder, num_images=args.num_images)
